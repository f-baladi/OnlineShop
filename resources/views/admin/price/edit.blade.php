...
@section('content')
    <div class="container">
        ...
        <div class="card">
            <div class="card-header">{{ __('public.edit',['name'=> 'قیمت']) ." - ". $price->product->title}}</div>
            <div class="card-body">
                {!! Form::model($price,['url' =>  route('admin.prices.update',$price),'files'=>true]) !!}
                @csrf
                @method('put')
                ...
                <button class="btn btn-success">{{__('public.edit',['name'=>'قیمت'])}}</button>
                {!! Form::close() !!}
